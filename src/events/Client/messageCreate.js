...
module.exports = {
  ...
  run: async (client, message) => {
    ...
    const args =
      np.includes(message.author.id) === false ? message.content.slice(pre.length).trim().split(/ +/) :
        message.content.startsWith(pre) === true ? message.content.slice(pre.length).trim().split(/ +/) :
    ...
    if (command.voteonly) {
      ...
      if (!vote) {
        let embed = new MessageEmbed()
          .setDescription(`<:online:1210253399031812147> | **This Is A Vote Required Command So Vote Us Now By [Clicking Here](https://top.gg/bot/${client.user.id}/vote)**`)
          .setColor(client.embedColor);
    ...
    if (!message.guild.me.permissions.has(Permissions.FLAGS.SEND_MESSAGES)) {
      await message.author.dmChannel.send({
        content: `${i18n.__mf("events.msgcrt.embed2", { channelId: message.channel.id, cmdname: command.name })}`
      }).catch(() => {});
    ...
    if (!message.guild.me.permissions.has(Permissions.FLAGS.EMBED_LINKS)) {
      await message.channel.send({
        content: `${i18n.__mf("events.msgcrt.embed3", { cmdname: command.name })}`
      }).catch(() => {});
    ...
    if (!channel.permissionsFor(message.guild.me)?.has(Permissions.FLAGS.EMBED_LINKS) && client.user.id !== message.author.id) {
      return channel.send({ content: `${i18n.__("events.msgcrt.embed5")}` });
    }

    if (command.owner) {
      if (!owner.includes(message.author.id)) {
        embed.setDescription("<:online:1210253399031812147> | Only My Developers Can Use This Command");
        return message.channel.send({ embeds: [embed] });
      }
    }
