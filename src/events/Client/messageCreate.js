    ...
    if (command.voteonly) {
      ...
      if (!vote) {
        let embed = new MessageEmbed()
          .setDescription(
            `<:king:1199357089395527723> | **This Is A Vote Required Command So Vote Us Now By [Clicking Here](https://top.gg/bot/${client.user.id}/vote)**`,
          )
          .setColor(client.embedColor);
    ...
    if (command.owner) {
      if (!owner.includes(message.author.id)) {
        embed.setDescription(
          "<:cross:1200733725344944178> | Only My Developers Can Use This Command",
        );
        return message.channel.send({ embeds: [embed] });
      }
    }
