  ...
  execute: async (message, args, client, prefix) => {
    ...
    try {
      ...
      return message.reply(
        `<:cross:1200733725344944178> | there was an error while searching: ${err.message}`,
      );
    }
    switch (res.loadType) {
      case "NO_MATCHES":
        ...
        return message.channel.send({
          embeds: [
            new MessageEmbed()
              ...
              .setDescription(
                `<:icons_search:1200736180514340925> | No matches found for - ${search}`,
              ),
          ],
        });
      case "TRACK_LOADED":
        ...
        if (!player.playing && !player.paused && !player.queue.size) {
          ...
          const thing = new MessageEmbed()
            ...
            .setDescription(
              `${i18n.__("player.addq")}\n[${track.title}](https://discord.gg/k7euMVR4q6) - \`[${convertTime(track.duration)}]\``,
            );
